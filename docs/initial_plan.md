...
| `LEDR[17:0]` | 目前行進度條 / 存檔動畫        | 平時依目前行 / 文件總行數，從 `LEDR17` 往 `LEDR0` 亮；最後一行才亮 `LEDR0`。EEPROM 寫入期間暫時使用 `LEDR17..LEDR1` 單燈跑馬燈 |
...
內容修改後只設定 dirty_flag
按下 KEY0 時才寫入 EEPROM
EEPROM 寫入期間以 LEDR17..LEDR1 顯示單燈跑馬燈；這只是儲存中視覺效果，不代表儲存進度
存檔成功後清除 dirty_flag
...
| LED          | 顯示內容               |
| ------------ | ------------------ |
| `LEDR[17:0]` | 平時顯示目前行 / 文件總行數進度，從 LEDR17 亮到 LEDR0；存檔時暫時以 LEDR17..LEDR1 顯示跑馬燈 |
...
4. 編輯後設定 dirty_flag
5. KEY0 手動儲存文件
6. 寫入 EEPROM 期間顯示 LEDR17..LEDR1 單燈跑馬燈視覺效果
