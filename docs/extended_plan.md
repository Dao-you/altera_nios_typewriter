...
- SD 卡格式化為 FAT32。
- 從根目錄讀取固定文字檔。
- 檔名先採用 8.3 格式，例如 `QUESTION.TXT` 與 `EDITOR.TXT`。
- 文字內容先限制為英文 ASCII。
- 一行代表一筆題目。
- 題庫檢視與打字遊戲仍以 read-only `QUESTION.TXT` 為主；目前 editor 擴充已支援根目錄固定短檔名 `EDITOR.TXT` 的簡化讀寫。
...
FAT32 Reader
    ↓
Find QUESTION.TXT / EDITOR.TXT
    ↓
Read Text Content
    ↓
Question Buffer
    ↓
Typing Game / Text Display / SD Editor
```
...
目前可以先完成最低限度的根目錄 8.3 檔名功能：

1. 初始化 SDHC。
2. 成功讀取 sector 0。
3. 解析 MBR / Boot Sector。
4. 找到 FAT32 root directory。
5. 搜尋 `QUESTION.TXT` 或 `EDITOR.TXT`。
6. 讀取檔案內容。
...
| Sector 讀取 / 單 block 寫入 | C + SPI driver | 固定 8.3 檔名與根目錄流程較容易測試 |
...
- 可以找到根目錄中的 `QUESTION.TXT`。
- 可以讀出第一行題目並顯示。
- 題目資料可以提供給後續打字遊戲使用。
- SD editor 可找到或建立根目錄中的 `EDITOR.TXT`，並以 ASCII 文字載入 / 儲存。
...
SDHC 文字檔讀取則適合放在第二階段。初期建議使用 SDHC + FAT32 + 固定 8.3 檔名 + ASCII 文字檔；目前 `QUESTION.TXT` 提供題庫載入，`EDITOR.TXT` 提供 SD editor 簡化讀寫。長檔名、子目錄、完整檔案管理與 SDXC/exFAT 仍屬於後續擴充。
