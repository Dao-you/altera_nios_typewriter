...
- `display_show_action_message(message, action_text)`：第一列顯示訊息，第二列顯示自訂按鍵提示，LEDR 使用 2 Hz 全燈閃爍。打字遊戲開始前的 `SW0-7 OFF` / `KEY1GO KEY0EXIT` 使用這個入口。
- `display_show_text_page(text, length, first_line)`：將 newline-delimited 文字以兩列 LCD 顯示，現在用於 SD `QUESTION.TXT` 測試畫面。
- `display_show_typing_game(question, question_len, input, current_round, total_rounds, elapsed_ms)`：打字遊戲主畫面。LCD 第一列顯示題目 viewport，第二列顯示共用 `EditorDocument` 輸入 viewport，兩列共用同一個水平捲動起點；LCD cursor 放在第二列輸入位置。LEDG 顯示目前題號 / 總題數進度，HEX7~HEX6 顯示目前題號，HEX5~HEX4 顯示總題數，HEX3~HEX0 顯示經過時間 `mmss`。
- `display_show_typing_done(total_rounds, elapsed_ms)`：打字遊戲完成畫面。LCD 顯示 `Game complete` / `KEY0 OK`，LEDR 使用 2 Hz 全燈閃爍，LEDG 顯示滿格進度，HEX 保留最終總題數與時間。
...
打字遊戲是例外的全列進度畫面：`display_show_typing_game()` / `display_show_typing_done()` 會由 display 層直接接管 LEDG7..LEDG0，依目前題號從 LEDG7 往 LEDG0 點亮。離開遊戲畫面後，其他 display API 會重新清空或設定一般 LEDG indicator。
...
- 打字遊戲：`main.c` 從首頁進入 `APP_STATE_TYPING_READY`，用 `display_show_action_message()` 提示關閉 SW0..7。開始後用 SD activity marquee 讀取 `QUESTION.TXT`，`typing_game.c` 從非空行隨機抽 10 題。遊戲中 `editor_input.c` 共用 editor 的 SW/KEY/PS2 輸入派發，但禁用 LF 以維持單行答案；`KEY0` 開啟 `Quit` / `Restart` / `Continue` 暫停選單。時間目前由主迴圈 10 ms tick 累加，若要精準比賽計時，應在 Qsys 加 interval timer 並更新 BSP 後改由硬體 tick 驅動。
