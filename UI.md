...
- `display_update_eeprom_editor(editor, ascii, nav_mode, eeprom_error)`：EEPROM editor 主畫面入口。第一列顯示置中的閃爍 `EEPROM` marker，第二列顯示目前編輯行並放置 LCD cursor。
...
- Modal 訊息：`main.c` 以 `APP_STATE_INFO_MESSAGE`、`APP_STATE_CONFIRM_MESSAGE`、`APP_STATE_ERROR_MESSAGE` 包裝 display 層的互動訊息。`Save to ROM` 會用一般/錯誤訊息回報結果，`Clear All` 會先進確認訊息，EEPROM 載入失敗會進錯誤訊息。
...
- EEPROM editor 主畫面：每輪由 `display_update_eeprom_editor()` 統一刷新，不在 `main.c` 個別操作 LED 或 LCD。
