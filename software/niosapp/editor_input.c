                              ...
                              int allow_newline)
{
    if (key_pressed_edge(keys, KEY_MASK_1)) {
        (void)editor_input_write_ascii(editor, ascii, allow_newline);
    }
    if (key_pressed_edge(keys, KEY_MASK_3)) {
        if (nav_mode) {
            (void)editor_move_up(editor);
        } else {
            (void)editor_move_left(editor);
        }
    }
    if (key_pressed_edge(keys, KEY_MASK_2)) {
        if (nav_mode) {
            (void)editor_move_down(editor);
        } else {
            (void)editor_move_right(editor);
        }
    }
}
...
void editor_input_drain_keyboard(EditorDocument *editor, int allow_newline)
{
    ...
    while (count < EDITOR_INPUT_DRAIN_LIMIT && keyboard_read(&code)) {
        (void)editor_input_apply_code(editor, code, allow_newline);
        ++count;
    }
}
